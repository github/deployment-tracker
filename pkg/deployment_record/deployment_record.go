...
func NewDeploymentRecord(name, digest, version, logicalEnv, physicalEnv, cluster, status, deploymentName string) *DeploymentRecord {
	// Validate status
...
// ExtractImageName extracts the image name and tag from a container image reference.
// Returns the image name (without tag or digest) and the tag (or empty string if no tag).
...
func ExtractImageName(image string) (string, string) {
	...
	// But be careful with port numbers in registry URLs like "localhost:5000/image:tag"
	...
	// This handles cases like "localhost:5000/image" where we don't want to extract ":5000" as tag
